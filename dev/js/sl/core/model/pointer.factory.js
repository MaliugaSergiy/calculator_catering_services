(function (angular) {
    ...
    angular.module('angularAwesomeSlider').factory('sliderPointer', ['sliderDraggable', 'sliderUtils', function (Draggable, utils) {
        ...
        SliderPointer.prototype.onmousedown = function (evt) {
            ...
            this._parent = {offset: offset, width: offset.width, height: offset.height};
        ...
        SliderPointer.prototype.onmousemove = function (evt, x, y) {
            ...
            if (this.settings.realtime && this.settings.cb && angular.isFunction(this.settings.cb) && this.allowed) this.settings.cb.call(this.parent, this.parent.getValue(), !this.is.drag);
        };
        SliderPointer.prototype.onmouseup = function (evt) {
            if (this.settings.cb && angular.isFunction(this.settings.cb) && this.allowed) this.settings.cb.call(this.parent, this.parent.getValue(), !this.is.drag);
            if (!this.is.drag) this.ptr.removeClass("jslider-pointer-hover");
        };
        SliderPointer.prototype.limits = function (x) {
            return this.parent.limits(x, this);
        };
        SliderPointer.prototype.calc = function (coords) {
            return !this.vertical ? this.limits(((coords - this._parent.offset.left) * 100) / this._parent.width) : this.limits(((coords - this._parent.offset.top) * 100) / this._parent.height);
        };
        ...
        SliderPointer.prototype._set = function (prc, opt_origin) {
            ...
            if (this.threshold && this.parent.o.pointers[1]) {
                var v1 = this.parent.o.pointers[0].value.origin, v2 = this.parent.o.pointers[1].value.origin;
                this.allowed = (this.normalOrder ? v2 - v1 : v1 - v2) >= this.threshold;
            }
            if (!this.allowed) {
                var otherPtr = this.parent.o.pointers[this.uid === 0 ? 1 : 0];
                if (this.uid === 0) this.value.origin = this.normalOrder ? otherPtr.value.origin - this.threshold : otherPtr.value.origin + this.threshold; else
                    this.value.origin = this.normalOrder ? otherPtr.value.origin + this.threshold : otherPtr.value.origin - this.threshold;
                return;
            }
            this.value.prc = prc;
            if (!this.vertical) this.ptr.css({left: prc + "%"}); else
