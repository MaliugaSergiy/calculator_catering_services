(function (angular) {
    ...
    angular.module('angularAwesomeSlider').factory('slider', ['sliderPointer', 'sliderConstants', 'sliderUtils', function (SliderPointer, sliderConstants, utils) {
        ...
        Slider.prototype.init = function (inputNode, templateNode, settings) {
            ...
            if (this.settings.calculate && angular.isFunction(this.settings.calculate)) this.nice = this.settings.calculate;
            if (this.settings.onstatechange && angular.isFunction(this.settings.onstatechange)) this.onstatechange = this.settings.onstatechange;
        ...
        Slider.prototype.create = function (templateNode) {
            ...
            var divs = this.domNode.find('div'), is = this.domNode.find('i'), angElt = angular.element,
                angExt = angular.extend, angForEach = angular.forEach, pointer1 = angElt(divs[1]),
                pointer2 = angElt(divs[2]), pointerLabel1 = angElt(divs[5]), pointerLabel2 = angElt(divs[6]),
                indicatorLeft = angElt(is[0]), indicatorRight = angElt(is[1]), range = angElt(is[2]),
                indicator1 = angElt(is[3]), indicator2 = angElt(is[4]), indicator3 = angElt(is[5]),
                indicator4 = angElt(is[6]), pointers = [pointer1, pointer2], off = utils.offset(this.domNode),
                offset = {
                    left: off.left,
                    top: off.top,
            ...
            angExt(this.o.labels[0], {value: this.o.labels[0].o.find("span")});
            angExt(this.o.labels[1], {value: this.o.labels[1].o.find("span")});
            ...
            angForEach(pointers, function (pointer, key) {
                ...
                var value = values[key], prev, prevValue, test, value1, offset;
                if (value) {
                    ...
                    self.o.pointers[key].d = {left: offset.left, top: offset.top};
                }
            });
        ...
        Slider.prototype.clickHandler = function () {
            ...
            var resetPtrSize = function (ptr) {
                var ptr1 = self.o.pointers[0].ptr, ptr2 = self.o.pointers[1].ptr, offset1 = utils.offset(ptr1),
            ...
            return function (evt) {
                ...
                var vertical = self.settings.vertical, targetIdx = 0, _off = utils.offset(self.domNode),
                    firstPtr = self.o.pointers[0], secondPtr = self.o.pointers[1] ? self.o.pointers[1] : null, tmpPtr,
                    targetPtr, evtPosition = evt.originalEvent ? evt.originalEvent : evt,
                    mouse = vertical ? evtPosition.pageY : evtPosition.pageX, offset, css = vertical ? 'top' : 'left';
                ...
                if (secondPtr) {
                    ...
                    var middleGap = (secondPtr.d[css] - firstPtr.d[css]) / 2, ptr = firstPtr.d[css],
                        mousePosBetween = mouse - ptr, test = mousePosBetween > middleGap;
                    if (test) {
                ...
                targetPtr.onmouseup();
                angular.extend(targetPtr.d, {left: coords.x, top: coords.y});
        ...
        Slider.prototype.generateScale = function () {
            if (this.settings.scale && this.settings.scale.length > 0) {
                var str = "", s = this.settings.scale, prc, label, duplicate = {},
                    position = this.settings.vertical ? 'top' : 'left', i = 0;
                for (; i < s.length; i++) {
                    if (!s[i].val) {
        ...
        Slider.prototype.redrawLabels = function (pointer) {
            function setPosition(label, sizes, prc) {
                ...
                if (!self.settings.vertical) label.o.css({
                    left: prc + "%",
                    marginLeft: sizes.margin + "px",
                    right: "auto"
                }); else
                    label.o.css({top: prc + "%", marginLeft: "20px", marginTop: sizes.margin, bottom: "auto"});
                if (sizes.right && self.sizes.domWidth > 0) {
                    if (!self.settings.vertical) label.o.css({left: "auto", right: 0}); else
                        label.o.css({top: "auto", bottom: 0});
                }
                return sizes;
            }

            var self = this, label = this.o.labels[pointer.uid], prc = pointer.value.prc,
            ...
            var anotherIdx = pointer.uid === 0 ? 1 : 0, anotherLabel, anotherPtr;
            if (!this.settings.single && !this.settings.vertical) {
                anotherLabel = this.o.labels[anotherIdx];
                anotherPtr = this.o.pointers[anotherIdx];
                var label1 = this.o.labels[0], label2 = this.o.labels[1], ptr1 = this.o.pointers[0],
                    ptr2 = this.o.pointers[1], gapBetweenLabel = ptr2.ptr[0].offsetLeft - ptr1.ptr[0].offsetLeft,
        ...
        Slider.prototype.redrawLimits = function () {
            if (this.settings.limits) {
                var limits = [true, true], i = 0;
                for (var key in this.o.pointers) {
                    if (!this.settings.single || key === 0) {
                        var pointer = this.o.pointers[key], label = this.o.labels[pointer.uid],
                            label_left = label.o[0].offsetLeft - this.sizes.domOffset.left, limit = this.o.limits[0];
        ...
        Slider.prototype.getValue = function () {
            ...
            angular.forEach(this.o.pointers, function (pointer, key) {
                if (pointer.value.prc !== undefined && !isNaN(pointer.value.prc)) value += (key > 0 ? ";" : "") + $this.prcToValue(pointer.value.prc);
            });
            return value;
        };
        ...
        Slider.prototype.prcToValue = function (prc) {
            ...
            if (this.settings.heterogeneity && this.settings.heterogeneity.length > 0) {
                var h = this.settings.heterogeneity, _start = 0, _from = this.settings.from, i = 0;
                for (; i <= h.length; i++) {
                    var v;
                    if (h[i]) v = h[i].split("/"); else
        ...
        Slider.prototype.round = function (value) {
            ...
            if (this.settings.round) value = Math.round(value * Math.pow(10, this.settings.round)) / Math.pow(10, this.settings.round); else
